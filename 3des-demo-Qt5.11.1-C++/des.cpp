 ...
 * InB:    输入的Bit流[in]
 * loop:   Bit流的长度    返回值 说明：   void
 * 作       者:    邹德强   更 新 日 期：  2003.12.19
 * /*******************************************************************/
...
/*******************************************************************/
/*   函 数 名 称:  F_func   功 能 描 述：  实现数据加密到输出P
 * 参 数 说 明：  Out:    输出的32Bit[out]
 * In:     输入的48Bit[in]    返回值 说明：   void
 * 作       者:    邹德强   更 新 日 期：  2003.12.19
 * /*******************************************************************/
...
bool CDES::RunDes(bool bType,bool bMode,const unsigned char* In,unsigned char* Out,unsigned datalen,const unsigned char* Key,unsigned keylen)  {
    //判断输入合法性
    ...
    {
        unsigned char  cvec[8] = {1,2,3,4,5,6,7,8}; //扭转向量
...
/*******************************************************************/
/*   函 数 名 称:  RunPad   功 能 描 述：  根据协议对加密前的数据进行填充
 * 参 数 说 明：  bType   :类型：PAD类型
 * In      :数据串指针                 Out     :填充输出串指针
 * datalen :数据的长度                 padlen  :(in,out)输出buffer的长度，填充后的长度
 * 返回值 说明：   bool    :是否填充成功
 * 作       者:    邹德强   修 改 历 史：      更 新 日 期：  2003.12.19
 * /*******************************************************************/
bool CDES::RunPad(int nType, const unsigned char* In,unsigned datalen, unsigned char* Out, unsigned& padlen)  {
